...
import { sendVideoRemovedEvent } from "../kafka/handlers/videoRemovedEvent.producer";
...
export const deleteVideoController: RequestHandler = async (req, res) => {
	...
	try {
		...
		if (listResponse.Contents && listResponse.Contents.length > 0) {
			...
			logger.info(
				`Successfully deleted S3 objects for video ${videoId} from gcs/${videoId}/`
			);
		} else {
			logger.warn(
				`No S3 objects found for video ${videoId} in gcs/${videoId}/`
			);
		}
		...
		sendVideoRemovedEvent({ videoId, userId });
