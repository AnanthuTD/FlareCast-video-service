...
export class CreatePromotionalVideoUseCase implements ICreatePromotionalVideoUseCase {
  constructor(
  ...
  > {
    ...
    try {
      ...
      const promotionalVideo = await this.videoRepository.setPromotionalVideo(newVideo.id);

      // Generate S3 signed URL
      const key = `${newVideo.id}/original.${videoExtension}`;
      const signedUrl = await this.s3Service.generateUploadUrl(key, `video/${videoExtension}`);
