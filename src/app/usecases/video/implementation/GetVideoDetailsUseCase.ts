...
import { VideoCategory } from "@/domain/entities/Video";
...
export class GetVideoDetailsUseCase implements IGetVideoDetailsUseCase {
	...
	async execute(dto: GetVideoDetailsDTO): Promise<
		ResponseDTO & {
	...
	> {
		...
		try {
			...
			if (
				!video.isPublic &&
				video.userId !== userId &&
				video.category === VideoCategory.DEFAULT
			) {
				if (!video.spaceId) {
				...
				if (!isMember) {
					logger.debug(
						"User is not a member of the space and has no permission"
					);
					return {
						success: false,
						data: { error: GetVideoDetailsErrorType.UNAUTHORIZED },
